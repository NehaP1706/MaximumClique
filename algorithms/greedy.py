...
def find_maximum_clique_from_dict(G_dict):
    start_time = time.time()
    nodes = list(G_dict.keys())

    # Compute degrees and sort nodes by descending degree
    degrees = {node: len(G_dict[node]) for node in nodes}
    nodes_sorted = sorted(nodes, key=lambda x: degrees[x], reverse=True)
    ...
    for node in nodes_sorted:
        ...
        for other in nodes_sorted:
            if other == node:
                continue
            # Check if 'other' is connected to all members of clique
            if all(member in G_dict[other] for member in clique):
                clique.append(other)
